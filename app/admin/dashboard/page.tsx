...
export default function AdminDashboard() {
  ...
  async function fetchDashboardData() {
    try {
      // Fetch user count from auth.users using admin API
      const { data: { users }, error: usersError } = await supabase.auth.admin.listUsers()
      
      // Fetch counts from your tables
      const [coursesRes, programsRes, paymentsRes] = await Promise.all([
        supabase.from('courses').select('*', { count: 'exact', head: true }),
        supabase.from('quick_programs').select('*', { count: 'exact', head: true }),
  ...
  const statCards = [
    ...
    { 
      label: 'Quick Programs', 
      value: stats.totalPrograms, 
      color: 'from-fuchsia-500 to-pink-500', 
      icon: Layout,
      trend: '+5%',
      trendUp: true,
      href: '/admin/quick-programs'
    },
    { 
      label: 'Revenue', 
  ...
  const quickLinks = [
    ...
    { label: 'Add Program', href: '/admin/quick-programs', icon: Layout, color: 'bg-fuchsia-600 hover:bg-fuchsia-700' },
